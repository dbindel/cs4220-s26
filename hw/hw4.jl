...
# ╔═╡ 42d11803-e174-42f5-84c3-c1b8524a7e89
md"""
## 1. Dear colleagues
...
# ╔═╡ 8996add0-e0db-448c-b195-faeed9bd0ab9
"""
    a = fit_cheb_series(d, f)

Fit a Chebyshev series with a vector of coefficients `[a0 ... ad]` for a
univariate function `f`.
"""
...
# ╔═╡ c2fa294f-91c8-46c7-8d4b-f7d56a4a7fb8
md"""
It is convenient to have a corresponding function to evaluate a Chebyshev series,
too.
"""

# ╔═╡ c6f22b87-b23c-4a14-b4b1-f68b15bc56c6
"""
    fhatx = eval_cheb_series(a, x)

Evaluate a Chebyshev series with coefficients `a` at a point `x`.
"""
...
# ╔═╡ 4e2dd287-197f-4da8-ba16-a4b8c6ccd0ab
md"""
...
2. Write a function `root_cheb_series(a)` that computes all the real zeros of $\hat{f}(x)$ between $[-1, 1]$ by solving an eigenvalue problem with the colleague matrix.  Note that the Julia [`eigvals`](https://docs.julialang.org/en/v1/stdlib/LinearAlgebra/#LinearAlgebra.eigvals) routine computes the eigenvalues of a matrix.
...
# ╔═╡ 93801a05-723a-4a72-9264-eea9394f223f
# ╠═╡ disabled = true
#=╠═╡
let
        f(x) = sin(5*exp(x))
...
# ╔═╡ f3be2bd6-24df-4f04-ae1a-7e165bd8b44d
md"""
## 2. Twirling, twirling
...
$$A^k \begin{bmatrix} x & y \end{bmatrix} \begin{bmatrix} c_1 \\ c_2 \end{bmatrix} = \begin{bmatrix} x & y \end{bmatrix} \rho^k \begin{bmatrix} \cos(k\theta) & \sin(k\theta) \\ -\sin(k\theta) & \cos(k\theta) \end{bmatrix} \begin{bmatrix} c_1 \\ c_2 \end{bmatrix}$$
...
# ╔═╡ 8d63bbec-1ce1-4ec4-95a4-78e648aec594
md"""
## 3. Midterm feedback

The oral midterm exam was an experiment, and I am considering whether to do it again the next time I teach this class (earliest will be Spring 2028).  Apart from the logistical issues (10 minutes was too short, and it takes a lot of time even with that), I am curious about your thoughts.  For two points:

1. Do you think an oral midterm makes sense (vs a conventional evening prelim)?  Why or why not?  Are there things that could make such a format more effective?

2. Suggest something that you think would have made a good problem for a similar midterm in the future.  You may also suggest a follow-up question if you feel so inclined.
"""
