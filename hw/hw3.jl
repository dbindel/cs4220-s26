...
where $A \in \mathbb{R}^{n \times n}$ is already factored via `F = lu(A)` and $b, c, f \in \mathbb{R}^n$ and $d \in \mathbb{R}$ are given as inputs.  Without worrying about numerical stability, implement a function

    function p1solve(F, b, c, d, fg)  # Returns [x; y]

You should be able to test the result with the following function.
"""

# ╔═╡ 1ab03de0-ad22-4894-87e1-9bd2924b5d76
function p1tester(n=10)

	# Set up random test problem
	A = rand(n,n)
	b = rand(n)
	c = rand(n)
	d = rand()
	...
	# Solve two ways and compare
	xy_ref = [A b; c' d]\fg
	xy = p1solve(F, b, c, d, fg)
	norm(xy-xy_ref)/norm(xy_ref)
end

# ╔═╡ 5af2ab0d-b878-49fc-90f0-e7b76c1b7ae1
md"""
## 2. Schur thing

Suppose a symmetric matrix $M \in \mathbb{R}^{n \times n}$ can be written as
...
Argue that this means that if $M$ is positive definite, then so is $S$.
"""

# ╔═╡ 0c5e5054-abd6-4565-9eb1-0e75dff20a37
md"""
## 3. Refined tastes
...
function p3data()
	A = [ 0.12252   0.643534  0.698335  0.509952  0.94854;
		  0.508435  0.465356  0.201126  0.8296    0.65526;
 	  	  0.73855   0.542969  0.591759  0.174934  0.698204;
	  	  0.207833  0.693395  0.338511  0.594114  0.856577;
	  	  0.394722  0.579533  0.181927  0.98999   0.133018]
	b = [0.955803; 0.695305; 0.170687; 0.381087; 0.900529]
	A, lu(Float16.(A)), b
end
