...
#=
---
subtitle: Derivative free methods
date: 2026-05-01
engine: jupyter
---
...
the gradient estimate increases (@fig-simplex-err).

```{julia}
#|echo: false
#|output: true
#|label: fig-simplex-err
#|fig-cap: Simplex gradient estimate error.  As $y$ approaches 1, the simplex approaches degeneracy (and the linear system we have to solve approaches singularity).
let
    ...
    plot(ys, errs, xlabel=L"y", ylabel="Simplex rel error", yscale=:log10)
end
```
...
problem with the code below (@fig-hooke-jeeves).

```{julia}
#|echo: false
#|output: true
#|label: fig-hooke-jeeves
#|fig-cap: Convergence of the Hooke-Jeeves algorithm for a quadratic model problem.
let
    H = [4 1; 1 3]
