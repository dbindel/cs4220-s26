---
subtitle: Ill-posedness and regularization
date: 2026-02-25
---
...
$$
  \mbox{perimeter} = 
  \alpha \cdot \mbox{side length} + 
  \beta \cdot \mbox{diagonal}.
$$
...
$$
  ...
  b = 4 s
$$
where $s$ is a vector of side lengths. The normal equations
are therefore
$$
  A^T A = \|s\|^2
...
than one. The equations are singular, but consistent. We have no data
that would lead us to prefer to write $p = 4s$ or $p = 2 \sqrt{2} d$
or something in between.  The fitting problem is *ill-posed*.
...
can connect concretely to the type of sensitivity analysis we have seen
before, a task we turn to now.
...
$$
  Ax \approx b.
$$
We partition $A$ and $b$ into the first $m$ rows
...
$$
The term $\|\hat{r}\|^2$ is the (squared) bias term, the part of the error that
...
process. If we dig further into this, we can see that
$$\begin{aligned}
...
$$
  ...
  \|A_1^\dagger\| \|e\|.
$$
If there were no measurement error $e$, we
...
This leads to the *pivoted QR factorization*
$$
  A \Pi = Q R
$$
...
$$
If we know of no particular problem structure in advance, the standard
...
regularized problem has the property that the solutions tend to become
sparse as $\lambda$ becomes larger. That is, the $\ell^1$
...
algebra. Instead, one treats it as a more general *convex
optimization* problem. We will discuss some approaches to the solution
of such problems later in the semester.
