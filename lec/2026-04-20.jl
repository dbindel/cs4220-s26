...
a sample quadratic model (@fig-demo-tr1).

```{julia}
#| echo: false
#| output: true
#| label: fig-demo-tr1
#| fig-cap: Plot of trust region subproblem solve for varying $\Delta$.
let
...
is a piecewise linear approximation to the true path (@fig-demo-dogleg).

```{julia}
#| echo: false
#| output: true
#| label: fig-demo-dogleg
#| fig-cap: Plot of trust region subproblem dogleg approximation for varying $\Delta$.
let
...
begin
    ...
    p1 = plot(rhist, yscale=:log10, label=L"\|\nabla \phi(x_k) \|")
    p2 = plot(Δhist, yscale=:log10, label=L"\Delta")
    plot!([norm([xhist[i+1]-xhist[i]; yhist[i+1]-yhist[i]])
           for i in 1:length(xhist)-1], label="step")
    p3 = plot(ϕhist, label=L"\phi(x_k)")
