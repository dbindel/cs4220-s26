...
There is thus a close connection between nonlinear equation solving on
the one hand and optimization on the other, and methods used for one
problem can serve as the basis for methods for the other.

As with nonlinear equations, the one-dimensional case is the simplest,
and may be the right place to start our discussion. As with the solution
of nonlinear equations, our main strategy for dealing with
multi-variable optimization problems will be to find a promising search
direction and then solve (approximately) a one-dimensional line search
problem.

# Minimization via 1D Newton

Suppose $g : \bbR \rightarrow \bbR$ has at least two continuous
derivatives. If we can compute $g'$ and $g''$, then one of the simplest
ways to find a local minimum is to use Newton iteration to find a
stationary point:
$$
...
$$
  \hat{g}(x_{k+1}) = g(x_k) + g'(x_k)(x_{k+1}-x_k) + 
  \frac{1}{2} g''(x_k) (x_{k+1}-x_k)^2.
$$
...
$$
So if $g'(x_k) u > 0$, then $-u$ is a *descent direction*, and thus
...
# Approximate bisection and golden sections
...
*four* distinct points $a < x_1 < x_2 < b$? There are really two cases,
both of which give us progress.
...
minimum of that quadratic. This *method of successive parabolic
interpolation* does converge locally superlinearly. But even when $g$ is
unimodular, successive parabolic interpolation must generally be coupled
with something slower but more robust (like golden section search) in
order to guarantee good convergence.
...
2.  Suppose $f(x)$ is approximated on $[0,1]$ by a polynomial $p(x) =
      c_0 + c_1 x + \ldots + c_d x^d$, and we know that $|f(x)-p(x)| <
      \delta$ on the interval. Using MATLAB's `roots` function, how
