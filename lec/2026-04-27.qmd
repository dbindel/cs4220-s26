---
subtitle: Methods for rquality constraints
date: 2026-04-27
engine: jupyter
---
...
#| output: true
#| label: fig-test-qp
#| fig-cap: Example constrained quadratic problem.
let
...
#| output: true
#| label: fig-qp-ns
#| fig-cap: Minimization via null space method.  White point is a feasible solution
let
    xx = range(-3, 3, length=100)
    plot(xx, xx, (x,y) -> ϕ1([x; y]), st=:contour, legend=false)
    plot!(xx, 1.0 .- xx, linewidth=2)
...
The null-space approach is plotted in @fig-qp-ns.
...
let
    ...
    p = plot(xx, xx, (x,y) -> ϕ1([x; y]), st=:contour, legend=false,
             title = "Error at μ=$μ: $(norm(xhat-x_ns))")
    plot!(xx, 1.0 .- xx, linewidth=2)
    plot!([u_ns[1]], [u_ns[2]], markercolor=:white, marker=true)
...
#| echo: true
#| label: fig-penalties
#| fig-cap: Accuracy as a function of penalty parameters
let
...
in @fig-penalties.  The plot shows the tradeoff between
