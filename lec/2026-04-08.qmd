---
subtitle: Gradient descent and Newton for optimization
date: 2026-04-08
engine: jupyter
---
...
function plot_vs_approx(ϕ, ϕ̂, x0, xr=[-1.0; 1.0], yr=[-1.0; 1.0])
    ...
    plot!([x0[1]], [x0[2]], marker=true, label=L"x_0")
...
#|echo: false
#|label: fig-taylor1
#|fig-cap: Comparing functon to first-order Taylor approximation
let
...
#|echo: false
#|label: fig-taylor2
#|fig-cap: Comparing function to second-order Taylor approximation
let
...
#|output: true
#|label: fig-newton
#|fig-cap: Convergence of Newton iteration on example $\phi$.
let
    x = [0.0; 0.5]
...
Let's try an example using the test function above.  If we
play with $\alpha$, what happens to the convergence?

```{julia}
#|echo: true
#|output: true
#|label: fig-gradient
#|fig-cap: Convergence of gradient descent on example $\phi$.
let
    x = [0.0; 0.5]
    α=0.1
...
To illustrate the point, let's try plugging in the optimal $\alpha$
and some nearby values in this case to confirm that it gives about as
good convergence as we can manage.
