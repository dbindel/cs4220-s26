...
#=
---
subtitle: Line search and globalization
date: 2026-04-13
engine: jupyter
---
...
#=
We show convergence of backtracking Newton below.
...
let
    ...
    p2 = plot(αhist[xhist .!= 0.], label=L"\alpha")
    plot(p1, p2, layout=@layout [a; b])
end
```
...
begin
    ...
    plot(plot(xx3, ϕ3.(xx3), label=L"\phi"),
         plot(xx3, dϕ3.(xx3), label=L"\phi'"),
         layout=@layout [a; b])
end
```
