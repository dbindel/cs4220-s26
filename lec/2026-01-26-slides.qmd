...
subtitle: Review of Linear Algebra
date: 2026-01-26
format:
  revealjs:
...
p = plot(framestyle=:zerolines,
         ...
         legend=false)
for k = -5:5
    plot!(k .+ [-1, 1], [-2, 2], linewidth=3,
          linecolor=:black, linestyle=:dashdot)
end
plot!([-1, 1], [-2, 2], linewidth=5, linecolor=:blue)
p
```

## Subspace quotients

Identify $\calV / \calU$ with a subspace of *particular solutions*

```{julia}
#|echo: false
plot!([-6, 6], [0, 0], linewidth=5, linecolor=:red)
```

## Subspace quotients

Choice of particular solutions for $\calV / \calU$ is not unique!
...
plot!([-1, 1], [-2, 2], linewidth=5, linecolor=:blue)
plot!([-6, 6], [-1, 1], linewidth=5, linecolor=:red)
```
...
Example decomposition of $\bbR^2$
...
plot!([-1, 1], [-2, 2], linewidth=5, linecolor=:blue)
plot!([-6, 6], [-1, 1], linewidth=5, linecolor=:red)
plot!([6*0.3 + 1*0.4, 6*0.3], [1*0.3 + 2*0.4, 1*0.3],
      linewidth=4, linestyle=:dash, linecolor=:red)
plot!([6*0.3 + 1*0.4, 1*0.4], [1*0.3 + 2*0.4, 2*0.4],
      linewidth=4, linestyle=:dash, linecolor=:blue)
scatter!([6*0.3], [1*0.3], markersize=6, markercolor=:red)
scatter!([1*0.4], [2*0.4], markersize=6, markercolor=:blue)
scatter!([6*0.3 + 1*0.4], [1*0.3 + 2*0.4], markersize=6, markercolor=:black)
```
...
$S \subset \calV$ is *linearly independent* if any $v$ in the span is
written *uniquely* as
$$
  v = \sum_{j=1}^m \alpha_j s_j.
$$
Equivalent: $\sum_{j=1}^m \alpha_j s_j = 0 \iff \forall j, \alpha_j = 0$
...
Example: Power basis (monomial basis) for $\calP_2$ is
$$
...
Our three favorite norms for $\bbR^n$ (or $\bbC^n$):
$$\begin{aligned}
\|x\|_2 &= \sqrt{\sum_{j=1}^n |x_j|^2} & \mbox{Euclidean norm} \\
\|x\|_1 &= \sum_{j=1}^n |x_j| & \mbox{Manhattan norm}\\
\|x\|_\infty &= \max_{j=1}^n |x_j| & \mbox{max norm}
\end{aligned}$$
...
\end{aligned}$$
The *Gram matrix* $G$ of inner products is symmetric (Hermitian) and
positive definite.
...
- Weights $w_j = y_j^2/\|y\|^2$ are positive and sum to one.
- So the max possible value of the weighted average is $\sigma_1$
