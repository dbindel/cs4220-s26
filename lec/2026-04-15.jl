...
#=
---
subtitle: Quasi-Newton and other iterations
date: 2026-04-15
engine: jupyter
---
...
@fig-broyden-cvg.  We use the exact Jacobian at the starting point as
our initial Jacobian estimate.

```{julia}
#|echo: false
#|label: fig-broyden-cvg
#|fig-cap: Convergence of Broyden.
let
    rhist = []
...
#|echo: false
#|label: fig-rre-cvg
#|fig-cap: Convergence of reduced-rank extrapolated simple fixed-point.
let
    rhist0 = []
    rhistr = []
...
with $T_N$ as the Jacobian approximation in @fig-aa-cvg.

```{julia}
#|echo: false
#|label: fig-aa-cvg
#|fig-cap: Convergence of Anderson acceleration of a simple fixed-point.
let
    rhistaa = []
    J0 = Jautocatalytic(0*v0)
