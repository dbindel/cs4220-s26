...
#=
---
subtitle: Gauss-Newton and nonlinear least squares
date: 2026-04-10
engine: jupyter
---
...
Like linear least squares, nonlinear least squares problems often
arise from model fitting.  Some important special cases include
...
general nonlinear least squares problem, one separable problem, and
one robust regression problem involving a nonlinear loss function.  We
will use these examples to illustrate different ways we can use
problem structure -- and also show how to think about initial guesses.
...
$$
and while it is certainly possible to run an ordinary Newton iteration
for the optimization problem, we can also note that the second term is
potentially small in the case when we can solve to a small residual
(i.e. when $f(x)$ gets close to zero).  Hence, we consider the
*Gauss-Newton* iteration, a scaled gradient descent iteration with the
scaling matrix $J(x)^T J(x)$, i.e.
$$\begin{aligned}
  x^{k+1}
  &= x^k - \left[ J(x^k)^T J(x^k) \right]^{-1} J(x^k)^T f(x^k) \\
  &= x^k - J(x^k)^\dagger f(x^k)
\end{aligned}$$
As long as $J$ remains nonsingular, the Gauss-Newton direction is a
descent direction.  And assuming $J$ is Lipschitz with constant $L$
near a stationary point $x^*$, one can derive an error iteration
$$
  \|e^{k+1}\| \leq L \|J(x^*)^\dagger\|^2 \|f(x^*)\| \|e^k\| + O(\|e^k\|^2)
$$
This is an upper bound, but it suggests (accurately) that Gauss-Newton is
not guaranteed to converge even from small initial error.  At the same time,
convergence can be very rapid for well-behaved problems ($J(x^*)$ not too near
singular, Lipschitz constant is not too big) when the residual norm at the
solution is small.  If $f(x^*) = 0$, we recover the same type of quadratic
convergence we see for Newton's method.
=#
...
1. Show how to write one Gauss-Newton step in terms of a linear least
   squares problem.
2. Suppose $m = n$.  What does Gauss-Newton look like in this case?
3. Justify the claim that if $J$ is nonsingular, the Gauss-Newton
   direction is a descent direction.

# A reaction rate example

This example is taken from [Wikipedia](https://en.wikipedia.org/wiki/Gauss%E2%80%93Newton_algorithm#Example)
and involves fitting the chemical reaction rate for an enzyme mediated reaction.
The larger the substrate concentration of the enzyme ($[S]$), the faster the
rate of reaction $R$, according to a relation of the form
$$
...
residual by $\beta_2 + S_i$ to solve the ordinary least squares
problem
$$
...
As usual, it is useful to sanity-check that our Jacobian and Hessian are
implemented correctly.
=#

function test_jacobian(f, J, x; h=1e-6)
    δx = randn(size(x))
    ∂f_∂δx_fd = (f(x+h*δx)-f(x-h*δx))/(2*h)
    ∂f_∂δx = J(x)*δx
    norm(∂f_∂δx_fd-∂f_∂δx)/norm(∂f_∂δx)
end

function test_jacobian(fJ, x; h=1e-6)
    δx = randn(size(x))
    ∂f_∂δx_fd = (fJ(x+h*δx)[1]-fJ(x-h*δx)[1])/(2*h)
    ∂f_∂δx = fJ(x)[2]*δx
    norm(∂f_∂δx_fd-∂f_∂δx)/norm(∂f_∂δx)
end

#ldoc off
test_jacobian(biox_f, biox_J, biox_β0)
test_jacobian((x) -> biox_J(x)'*biox_f(x), biox_H, biox_β0)
#ldoc on
#=
```{julia}
#|echo: true
...
Having reassured ourselves that we have a good initial guess and a
correct Jacobian, let's apply both Gauss-Newton and Newton to solve
the problem.
...
Compared to the Newton iteration, Gauss-Newton is getting (fast)
linear convergence, and ends up taking about twice as many iterations
...
just that the algebra is a bit tedious.  We'll keep it in the notes
for the sake of completeness.

In the interest of keeping the notation concise, let's write $J^* =
J(x^*)$ and $J_k = J(x^k) = J_* + E_k$.  Similarly, let $f_* =
f(x^*)$.  We assume throughout the derivation that $J$ is Lipschitz
...
$$
  ...
  \leq \frac{L\|e^k\|^2}{2(\sigma_{\min}(J_*)-L\|e^k\|)}
$$
...
algorithm replaces the least squares solve in the Gauss-Newton
algorithm with a regularized least squares solve:
$$
  x^{k+1} = x^k - (J_k^T J_k + \lambda^2 D_k^2)^{-1} J_k^T f(x^k)
$$
The scaling matrix $D_k$ may be an identity (suggested by Levenberg)
or a diagonal matrix whose diagonal entries are the column norms of
$J_k$ (suggested by Marquardt).
...
1. Justify the claims about the asymptotic behavior of
   Levenberg-Marquardt as $\lambda$ approaches 0 and $\infty$.
2. Explain the implementation of the Levenberg-Marquardt step in the code.
...
[Lorentzian](https://en.wikipedia.org/wiki/Cauchy_distribution) peak
(also known as a Breit-Wigner peak or a Cauchy distribution) with
center $x_0$, width parameter $\Gamma$, and amplitude parameter $c$
has the form
$$
  \frac{c}{\pi} \frac{\Gamma/2}{(x-x_0)^2 + (\Gamma/2)^2}.
$$
Our problem here is to fit empirical measurements to a sum of
Lorentzian peaks.  This problem occurs often enough that there are
specific approaches people take (we wouldn't always treat this as a
general nonlinear least squares problem), but we will only take
advantage of the partially linear nature of the problem, i.e. that we
are minimizing
$$
...
*linear* least squares problem in which the coefficient matrix depends
...
#ldoc off
test_jacobian(lorx_fJ, lorx_p0)
#ldoc on

#=
We start with a not-fantastic initial guess (@fig-lorx-guess).
Getting a good initial guess is
actually an interesting problem, but one that we will not address
here.  The converged solution is shown in @fig-lorx-lm,
and the convergence history in @fig-lorx-lm-cvg.  From
...
#|fig-cap: Initial guess for Lorentzian fitting example.
lorx_plot(lorx_p0)
```
...
#|fig-cap: Convergence of Levenberg-Marquardt for Lorentzian fitting example.
plot(lorx_resids, yscale=:log10,
     ylabel=L"\|J^T f(x_k)\|", xlabel=L"k",
     legend=false)
```

## Questions

1. Play with the parameter $\lambda$ in the Levenberg-Marquardt call.
   What happens?
2. Play with the initial guess.  Does convergence improve from a
   better starting guess?
3. Argue that the "true" signal looks like $p(x)/q(x)$ for low degree
   polynomials $p$ and $q$, where $q$ has complex roots $x_{c,i} \pm
   \iota \Gamma_i/2$.  We can find $p$ and $q$ by setting $p(x_i) -
   y_i q(x_i) \approx 0$ in a least squares sense; how would we
   implement this directly to get a better initial guess for the
   centers and widths of the Lorentzians?
...
The reduced problem can then be solved with any of our nonlinear least-squares
solvers.  This technique, known as variable projection, is [widely used in
a variety of applications](http://stacks.iop.org/IP/19/R1).

Minimizing the norm of $f_{\mathrm{proj}}$ is often nicer than minimizing
the norm of the original $f$.  Of course, in order to do this, we need to
be able to compute the Jacobian of $f_{\mathrm{proj}}$!  Fortunately, we have
lots of experience at this point with computing such derivatives.  Using
...
and while we can apply $Q$ and $Q^T$ quickly, we probably do not want
to explicitly form $Q_2$.  But we can evaluate the above expression
without explicitly forming $Q_2$ by
...
In the case of the peak-finding problem, each of the parameters
...
#=
Because there was plenty of room to make an algebra error in our
manipulations above, let's do our usual sanity check.

```{julia}
#|echo: true
#|output: true
test_jacobian(lorx_fJproj, lorx_p0[1:6])
```

```{julia}
#|echo: false
#|label: fig-lorx-vproj
#|fig-cap: Converged solution for Lorentzian fitting example with variable projection formulation.
begin
    lorx_resids_p = []
...
#|fig-cap: Convergence of Gauss-Newton for variable projection formulation of Lorentzian fitting example.
plot(lorx_resids_p, yscale=:log10,
     ylabel=L"\|J^T f(x_k)\|", xlabel=L"k", legend=false)
```
...
initial guess.  The converged solution is shown in @fig-lorx-vproj;
we show the convergence plot in @fig-lorx-vproj-cvg.

## Questions

1. Can you derive the expression for $\delta P$ yourself?
2. Explain the expression for the derivative of $f_{\mathrm{proj}}$
   with respect to $x_{c,i}$.
3. We claimed that $P = I-Q_1 Q_1^T = Q_2 Q_2^T$; why are these two
...
This can, of course, be converted to a nonlinear least squares problem, but
we will stick with this form.  Let $\psi$ denote the derivative of $\rho$;
then we have
$$\begin{aligned}
  \nabla \phi(x) &= A^T \psi(r) \\
  H_{\phi}(x) &= A^T \operatorname{diag}(\psi'(r)) A
\end{aligned}$$
where $\psi(r)$ and $\psi'(r)$ should be interpreted elementwise.  A Newton
step for this problem is
$$
...
There are interesting cases where this is a real issue.
However, an a slight modification avoids this problem and *is*
globally convergent for convex loss functions.
Define $W(f)$ to be the diagonal matrix of
weights $w_k = \psi(r_k)/r_k$; then $x$ is a stationary point
for $\phi$ if
$$
...
$$
In words, at each step we compute a new weighted least squares fit to
...
$$
  ...
  o(r_k) = r_k + o(r_k);
$$
hence, as with Gauss-Newton, this iteration is similar to Newton
iteration when the residuals are small.
...
1. Write the IRLS iteration in additive update form -- that is, what
   least squares problem does $p^k = x^{k+1}-x^k$ satisfy?  Compare to
   the Newton least squares problem.
2. Explain the line `ws[r .== 0] .= 0.0` in the `irls` routine.

# Robust regression

Least squares regression works well for Gaussian noise, but tends to
perform poorly in the presence of *outliers*.  Fitting procedures that
better tolerate outliers are generally known as *robust regression*
procedures.  The most popular robust regression techniques involve
solving a nonlinear least squares problem
$$
  \mbox{minimize } \sum_{j=1}^m \rho(r_j), \quad r = Ax-b
$$
for some loss function $\rho$ that grows more slowly than the squared
loss.  The coefficients from this estimation procedure are known
as [$M$-estimators](https://en.wikipedia.org/wiki/M-estimator).
...
on the assumption that non-outlier data is subject to normal noise, we
also return a scale factor of MAD/0.6745 (since for the standard
normal the expected value of the MAD is 0.6745).
...
using `rr_init` to get an initial guess and the iteratively weighted
least squares procedure to optimize the Tukey loss with an appropriately
chosen scale factor.
...
#|echo: false
#|label: fig-rrx-cvg
#|fig-cap: Convergence of IRLS.
plot(rrx_results[4], yscale=:log10, xlabel="k", ylabel="grad norm",
     legend=false)
```
...
\$$(@sprintf("%.3f", rrx_results[2]))\$ and an associated scale factor of
\$$(@sprintf("%.3f", rrx_results[1]))\$.
After $(length(rrx_results[4])) steps of IRLS,
we get a converged result with an estimation error of
\$$(@sprintf("%.3f", rrx_results[3]))\$.
""")
```
The convergence plot is shown in @fig-rrx-cvg.

## Questions

1. Will the `rr_init` routine fail if we unluckily draw a subset such
   that `As[s,:]` is singular?
2. Plot $\psi(r)/r$ at the converged solution.  Explain what you observe.
=#
