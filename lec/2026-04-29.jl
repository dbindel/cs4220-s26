...
#=
---
subtitle: Methods for inequality constraints
date: 2026-04-29
engine: jupyter
---
...
#|fig-cap: Example constrained quadratic problem and path taken by active set solver.
let
    xhist = []
    ...
    plot!([x[1] for x in xhist], [x[2] for x in xhist],
          ...
          title="Solution at [$(xsol[1]); $(xsol[2])]")
end
```
...
We plot the central path taken for our test problem in @fig-test-qp-ip.
=#
...
#|fig-cap: Example constrained quadratic problem with central path.
let
    x = [2.0; 1.0]
    p = plot_ex16_3()
