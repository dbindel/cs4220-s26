...
#=
---
subtitle: Homotopy and continuation
date: 2026-04-22
engine: jupyter
---
...
let
    ...
    p1 = plot(γs, vcenter,
              xlabel=L"\gamma", ylabel=L"v_{\mathrm{center}}",
              legend=false)
    p2 = plot(γs, λs,
              xlabel=L"\gamma", ylabel=L"\lambda_{\max}",
              legend=false)
...
let
    ...
    plot(γs, vcenter, xlabel=L"\gamma", ylabel=L"v_{\mathrm{center}}",
         legend=false)
end
```
